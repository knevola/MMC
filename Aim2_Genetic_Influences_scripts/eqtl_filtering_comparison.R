...
fem_results_sig_batch <- fem_results_SNPs %>%  filter(., p < 0.05)
male_results_sig_batch <- male_results_SNPs %>% filter(., p < 0.05)

# Add identifying column
fem_results_sig_batch$ID <- paste(fem_results_sig_batch$POS, ":", fem_results_sig_batch$miRNA, sep = "")
fem_results_sig$ID <- paste(fem_results_sig$POS, ":", fem_results_sig$miRNA, sep = "")
male_results_sig_batch$ID <- paste(male_results_sig_batch$POS, ":", male_results_sig_batch$miRNA, sep = "")
male_results_sig$ID <- paste(male_results_sig$POS, ":", male_results_sig$miRNA, sep = "")

# Venn Diagram
library(VennDiagram)
venn.diagram(x = list(Fem_Batch = fem_results_sig_batch$ID, Fem = fem_results_sig$ID), main ="Top 11 SNPs, miRNA, Female",filename = "Female_venn.tiff")
venn.diagram(x = list(Male_Batch = male_results_sig_batch$ID, Male = male_results_sig$ID), main ="Top 11 SNPs, miRNA, Male", filename = "Male_venn.tiff")

# Filter top positions/SNP for validation ###
fem_results_sig_val <- fem_results_sig %>% filter(., POS %in% c(115803375,240223080))
fem_results_sig__batch_val <- fem_results_sig_batch %>% filter(., POS %in% c(115803375,240223080))
venn.diagram(x = list(Fem_Batch = fem_results_sig__batch_val$ID, Fem = fem_results_sig_batch$ID), main ="Top 2 SNPs, miRNA, Female",filename = "Female_venn_val.tiff")

male_results_sig_val <- male_results_sig %>% filter(., POS %in% c(60001153,60026732))
male_results_sig__batch_val <- male_results_sig_batch %>% filter(., POS %in% c(60001153,60026732))
venn.diagram(x = list(Male_Batch = male_results_sig__batch_val$ID, Male = male_results_sig_batch$ID), main ="Top 2 SNPs, miRNA, Male",filename = "Male_venn_val.tiff")

write.csv(fem_results_sig__batch_val, "Female_Top2_SNPs_eQTL_sig.csv", row.names = F, quote = F)
