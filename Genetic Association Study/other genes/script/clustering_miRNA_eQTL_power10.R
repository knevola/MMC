...
mlr_ME <- function(positions, miRNAs, cov, data){
  ...
  for (i in 1:length(positions)){
    ...
    for(j in 1:length(miRNAs)){
      ...
      z$FDR <- p.adjust(z$`Pr(>|t|)`, method = "BH", n =  5)
      row.names(z)<- NULL
      mlr1 <- rbind(mlr1, z)
...
net = blockwiseModules(dataExpr, power = 10,
...
write.csv(modulebkdn, "Genes_per_module_nofiltergenetics10.csv")
...
write.csv(MEs, "miRNA_MEs_nofiltergenetics10.csv")
write.csv(geneModuleMembership, "miRNA_geneModuleMembership_nofiltergenetics10.csv")

genes_colors <- cbind(geneModuleMembership,mergedColors)
miRNA <- row.names(genes_colors)
genes_colors<- cbind(genes_colors, miRNA)
write.csv(genes_colors, "MiRNA_significance_module_membership_nofiltergenetics10.csv")
write.csv(moduleTraitCor, "miRNA_Module_trait_cor_nofiltergenetics10.csv")
write.csv(moduleTraitPvalue, "miRnA_Module_trait_pvalue_nofiltergenetics10.csv")
...
dissTOM <- 1- TOMsimilarityFromExpr(datExpr = dataExpr, power = 10)
plotTOM <- dissTOM^9
diag(plotTOM) =NA
sizeGrWindow(9,9)
TOMplot(plotTOM, geneTree, moduleColors, main = "Network heatmap plot, all genes")

write.csv(MEs, "ModuleEigengenes_geneticsgenetics10.csv", row.names = T, quote = F)
...
plotEigengeneNetworks(MEs, "", marDendro = c(0,4,1,2), marHeatmap = c(3,4,1,2), cex.lab = 0.8, xLabelsAngle
                      = 90)
write.csv(geneofinterest, "GenesofInterest_nofiltergenetics10.csv")

# Recalculate topological overlap if needed
TOM = TOMsimilarityFromExpr(dataExpr, power = 10);
...
chooseTopHubInEachModule(dataExpr, colorh = genes_colors$mergedColors,power = 10 )

colorh1 <- genes_colors$mergedColors
ADJ1=abs(cor(dataExpr,use="p"))^10
...
write.csv(imconnectblue1, "IMConnect_MM_bluegenetics10.csv", row.names = F)
