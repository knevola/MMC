...
load("chr22.RData")
tidy_gt <- chr22$gt
tidy_gt$gt_GT <- as.factor(tidy_gt$gt_GT)
tidy_gt$simpleM <- NA
tidy_gt$simpleM[tidy_gt$gt_GT == "0/0"] <- 2 
tidy_gt$simpleM[tidy_gt$gt_GT == "1/1"] <- 0 
tidy_gt$simpleM[tidy_gt$gt_GT == "0/1"] <- 1 
...
write.table(wide_data, file = "simpleM_allSNPs_postQC.txt", sep = " ", quote = F, row.names = F, col.names = F)
