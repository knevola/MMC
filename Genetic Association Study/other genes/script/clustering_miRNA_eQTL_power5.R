...
write.csv(modulebkdn, "Genes_per_module_nofiltergenetics5.csv")
...
write.csv(MEs, "miRNA_MEs_nofiltergenetics5.csv")
write.csv(geneModuleMembership, "miRNA_geneModuleMembership_nofiltergenetics5.csv")

genes_colors <- cbind(geneModuleMembership,mergedColors)
miRNA <- row.names(genes_colors)
genes_colors<- cbind(genes_colors, miRNA)
write.csv(genes_colors, "MiRNA_significance_module_membership_nofiltergenetics5.csv")
write.csv(moduleTraitCor, "miRNA_Module_trait_cor_nofiltergenetics5.csv")
write.csv(moduleTraitPvalue, "miRnA_Module_trait_pvalue_nofiltergenetics5.csv")
...
dissTOM <- 1- TOMsimilarityFromExpr(datExpr = dataExpr, power = 5)
plotTOM <- dissTOM^9
diag(plotTOM) =NA
sizeGrWindow(9,9)
TOMplot(plotTOM, geneTree, moduleColors, main = "Network heatmap plot, all genes")

write.csv(MEs, "ModuleEigengenes_geneticsgenetics5.csv", row.names = T, quote = F)
...
plotEigengeneNetworks(MEs, "", marDendro = c(0,4,1,2), marHeatmap = c(3,4,1,2), cex.lab = 0.8, xLabelsAngle
                      = 90)
write.csv(geneofinterest, "GenesofInterest_nofiltergenetics5.csv")

# Recalculate topological overlap if needed
TOM = TOMsimilarityFromExpr(dataExpr, power = 5);
...
chooseTopHubInEachModule(dataExpr, colorh = genes_colors$mergedColors,power = 5 )

colorh1 <- genes_colors$mergedColors
ADJ1=abs(cor(dataExpr,use="p"))^5
...
write.csv(imconnectblue1, "IMConnect_MM_bluegenetics5.csv", row.names = F)
...
write.csv(mlr_filter, "ME_eQTL_resultsgenetics5.csv", row.names = F, quote = F)
